...
pub fn get_file_path<P: AsRef<Path>>(
    ...
    let hash_input = if to_lower { file_name.to_lowercase() } else { file_name.to_string() };
    ...
    if let Some(parent_path) = parent { parent_path.join(result) } else { PathBuf::from(result) }
}
